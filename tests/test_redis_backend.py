...
def setup_my_library():
    ...
    settings.configure(
        BACKEND="redis",
        BACKEND_OPTIONS={
            # "host":"redis-14007.c74.us-east-1-4.ec2.redns.redis-cloud.com",
            # "port":14007,
            # "decode_responses":True,
            # "username":"default",
            # "password":os.environ.get("REDIS_PASSWORD")
        },
        DEFAULT_TTL=600,  # 3 minutes
    )
    print("My library setup complete.")


class TestRedisMemory:  # Use a class to group related tests
    ...
    def test_ttl(self):
        @timeit_return
        @memoize(verbose=True, ttl=5, scope=CacheScope.GLOBAL.value)  # TTL of 1 second
        def my_function():
            time.sleep(2)  # Simulate some work
            return "Hello"
    ...
    def test_capacity(self):
        ...
        @timeit_return
        @memoize(verbose=True, scope=CacheScope.GLOBAL.value)
        def my_function(x):
            time.sleep(2)  # Simulate some work
            return x * 2
    ...
    def test_ignore_args(self):
        @timeit_return
        @memoize(verbose=True, ignore_args=["y"], scope=CacheScope.GLOBAL.value)
        def my_function(x, y):
            time.sleep(2)
            return x * 2
    ...
    def test_multitenet_org(self):
        @timeit_return
        @memoize(
            verbose=True, collection_name="my_cole", scope=CacheScope.ORGANIZATION.value
        )
        def my_function(ctx):
            time.sleep(2)  # Simulate some work
            return "Hello, World!"
    ...
    def test_multitenet_usr(self):
        @timeit_return
        @memoize(verbose=True, collection_name="my_cole", scope=CacheScope.USER.value)
        def my_function(ctx):
            time.sleep(2)  # Simulate some work
            return "Hello, World!"
